...
from base.util.sms.sms import sms_manager, AbstractSMS
...
class BasePhoneValidator(object):
    ...
    """
    SMS = sms_manager.sms[sms_choice.VALIDATION]  # type: AbstractSMS
...
class SMSPhoneValidator(BasePhoneValidator):
    def generate_and_send(self, pn):
        v = self.base_generator()
        self.SMS.send_sms(pn, {'code': v})
        return v
